/**
 * Marker protocol between subprocess workers and the crash-recovery harness.
 * Workers append one JSON line per phase; the harness polls the file so it can
 * SIGKILL a worker at a precise point (right after the simulated provider
 * accepted a page and before any local commit).
 */
  ...
  return new Promise((resolve, reject) => {
    const tick = () => {
      ...
      if (Date.now() >= deadline) {
        reject(new Error(`marker not observed within ${timeoutMs}ms`));
        return;
      }
      setTimeout(tick, 25);
    };
