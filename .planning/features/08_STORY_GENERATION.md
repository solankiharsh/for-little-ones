...
- **Required first:** F-007 (selected concept), F-006 facts + F-003 profile (canonical facts fields incl. `locale`), the `GenerationStepExecution` interface (execution contract; F-010 provides the runtime), `StoryProvider` interface (architecture v2).
