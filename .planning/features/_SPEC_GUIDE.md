...
Decision: first inspect nothing (no code exists); then evaluate the actual jobs/queue need on a **durable execution substrate** (wording stays neutral — D019). Candidate classes: a PostgreSQL-backed queue, a Redis-backed queue (BullMQ-class), or a workflow engine only if the spike shows its guarantees are needed — select after the D014 spike; a Redis-backed class additionally requires an outbox/reconciliation bridge so DB and queue stay consistent.

Dependency inversion (D019): generation specs (F-008/F-009) expose `GenerationStep` units (idempotency keys, attempt budgets per step) and consume the **`DurableExecutionContract`/`GenerationStepExecution`** interface; F-010 implements the runtime and depends on the step units; F-028 provides the durability principles the contract encodes. A spec must never name the runtime a-generation-spec-rides-on as a hard dependency (F-008⇄F-010 and F-009⇄F-010 cycles), and F-028 must never be listed as a dependency of a feature it provides patterns to.
...
| Long jobs | A **durable execution substrate** with explicit job states (F-010/F-028/D019); candidate classes — PostgreSQL-backed, Redis-backed (BullMQ-class), a workflow engine only if the spike shows its guarantees are needed — are selected after the D014 spike; outbox/reconciliation required for a Redis-backed class; worker restart must not lose book state (D010). |
