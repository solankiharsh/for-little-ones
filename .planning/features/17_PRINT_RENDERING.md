...
- Quote resolution is a commerce operation, not a renderer endpoint: it obtains a transient `PrintQuote` for `printSpec`, quantity and destination. The renderer provides no approval-facing quote; provider capability belongs to the printing integration and the purchased quote is snapshotted on the order.
...
Pricing/delivery quotes are transient commerce data. `PrintCapability` is live printing-integration data, while canonical `PrintSpec`/`PrintPreflightContract` supplies the stable geometry rules used by approval (F-016) and the editor (F-014) without depending on this renderer; this adapter's `PrintProvider` never answers an approval-facing quote.
...
- `PRINT_RENDERING_JOB`: trigger = `BOOK_APPROVED` event (F-016) (or on paid order, see Decision §15); inputs = approvedRevisionRef + printSpec + adapterRef; steps = (1) manifest reconcile/checksum, (2) per-page geometry preflight (F-015 print-geometry subset), (3) layout engine run — text reflow at fixed metrics, illustration placement at print DPI, decorative elements composited, (4) cover+spine assembly via adapter formula, (5) PDF assembly (font embedding, colour profile set), (6) `validateArtifact` run (adapter), (7) mark VALIDATED.
...
- **Decision needed — render timing:** generate artifact at `APPROVED` (pre-payment; checkout is instant; minor cost for approvals that never order) vs on paid order (post-payment; saves cost, adds checkout latency + risk of surprise after payment). Recommendation: generate at `APPROVED`, validate on paid order — mark agreed in `DECISIONS.md` before F-018 work.
