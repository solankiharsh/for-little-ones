...
- **Privacy lock-in:** a one-time consent banner quoting F-025's current retention policy (the numbers are PROPOSED pending legal sign-off): "I understand Ava's photos are used to create this book and any later books made from her saved profile, to check likeness, and are processed by the providers listed in our data policy; they are stored under that policy's retention schedule (see how we use and delete photos) and I can delete them anytime with Delete everything now" with confirm/settings links — single source of truth lives in F-025, not re-drafted here.
