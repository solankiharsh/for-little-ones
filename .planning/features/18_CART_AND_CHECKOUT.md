...
Sandbox storefront scaffolding exists (2026-09-23, `feat/storefront-cart-demo`, still prototype/draft-eligible — NOT the F-018 build):

- `apps/web/src/commerce/` — headless Store API client (`StorefrontCommerceClient`: create cart, add approved line, configure shipping, read cart, `beginCheckout`), opaque approved-revision metadata guard, cart state, `CartDrawer` slide-up UI, unit tests (21 on this branch). Gift toggles are UI-only and never ride the line metadata.
- `apps/commerce/src/api/store/flo/checkout-cart/route.ts` — completes a storefront-built cart: customer/shipping attach, `validateCart` revalidation (again inside the `completeCartWorkflow` hook), local system payment, capture, reconcile/drain; idempotent per `x-flo-idempotency-key`.
- KEEP (refactor, behavior unchanged): the automated-approval `sandbox-checkout` route's claim/order-summary/mark-done/release logic was extracted into `apps/commerce/src/lib/checkout.ts` — reused by `checkout-cart`; the sandbox harness still passes.
- The Cart "checkout" seam is real: live sandbox verification creates a cart over the Store API, adds an opaque line, and completes to a £29.20/GBP-order per the F-018 semantics (charged total equals displayed). Price parity at completion is enforced by the `completeCartWorkflow` hook re-checking the payment-session amount against the cart total; the client's displayed unit price comes from the same sandbox catalogue fixture the seed uses.

What the scaffolding does NOT yet do (remains the F-018 build): real buyer auth (F-001 claim), format picker on the approval card (F-016 boundary), wallet/card via Stripe, gift-to/from shipping destinations, ETA sourced from PrintProvider windows (F-019), reorder. Demo data resolves the single sandbox approval (`approved_sandbox_1`); production resolves from a `GET /books/{id}/approval` Book-service boundary that does not exist yet (DEFER).

Delivery + gift-recipient at checkout (ADD, 2026-09-23, same branch): the drawer now collects a delivery address (GB postcode validation per §6 gates) and gift lines flow through the seam for real.

- `packages/commerce/src/index.ts` gains the canonical `parseCheckoutEnvelope` (shipping address + per-line `{ line_index, recipient_label, message }`); kept-in-sync duplicates in `apps/commerce/src/lib/checkout-envelope.ts` (server) and `apps/web/src/commerce/cart.ts` (web). All three lock the same invariants: exactly the declared fields, GB postcode, `recipient_label` ≤ 40, `message` ≤ 200, message needs a recipient, unique line indices.
- `POST /store/flo/checkout-cart` now requires the envelope: it rejects unknown body fields, validates the envelope fail-closed before the idempotency claim, attaches the real ship-to address to the cart, and persists `flo_envelope` (ship-to + gifts) onto the **order** — line metadata stays exactly `approvedBookRevisionId, contentHash, productFormatId, printSpecId` (privacy §7/§12: delivery + gift are household PII and never ride the line).
- `StorefrontCommerceClient.beginCheckout` validates fail-closed client-side before any request; the drawer gates Pay on the same validation (§6 gates before payment).
- Live sandbox e2e (temp script) asserts: charged total equals displayed (£29.20), envelope round-trips onto the order, gift/recipient never appear in line metadata, replay dedupes to the same order, and bad/unknown/out-of-range envelopes are refused without consuming the idempotency key.
- KEEP: this refines the existing scaffolding — no rewrite; cart-line metadata untouched; the sandbox harness still passes.
