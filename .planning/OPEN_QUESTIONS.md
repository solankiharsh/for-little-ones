...
- Which concept-generation path is canonical after the PR #25 / PR #26 merge? — **open.** Two implementations of F-007 now coexist: the durable `BookService` + `ConceptBundleRunner` pipeline from the M1 creation-core slice (`apps/api/src/creation/`, main's D023), and the commerce creation-project path (`api/generate-concepts.ts` + `flo_generation_job`, PR #25). The browser flow (`CreationFlow.tsx`) still calls the commerce path. Candidates: move the flow onto the durable pipeline and retire the commerce path, or keep a deliberately commerce-scoped path with a stated boundary. Note the two disagree on moderation (the durable path moderates the actual copy, F-007 §10) and on attempt accounting.
