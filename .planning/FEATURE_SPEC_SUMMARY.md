...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 01 | Onboarding & account claim | P0 | M6 | Explore before account; claim later | `anonymous_project_id` + browser token; magic-link claim | — | Unclaimed-project retention |
...
| 04 | Photo upload | P0 | M1 | Upload 1–5, gentle validation, delete anytime | Photo pipeline + face/resolution QA; API-relay upload | 03 | Rejecting usable photos; provider for face-detection |
...
| 10 | Generation progress | P1 | M1→M2 | Emotional progress; refresh-safe; resumable | DB-backed job/queue; step+page states | 08, 09, 28 | Queue substrate choice |
| 11 | Book preview | P0 | M1 (v0) | Premium reader; no editor chrome | Lightweight reader renderer; bundle-isolation guard | 08, 09 | Reader/editor bundle separation |
...
| 14 | Book editor | P1 | M3 | Simple editor escape hatch, not Canva | OpenPolotno wrapper + canonical↔snapshot adapter | 11, 12, 13 | OpenPolotno serialization stability |
| 15 | Book QA | P1 | M2→M4 | Problems flagged before it reaches you | QA suite (identity/facts/layout/print gates), re-runnable | 09, 11, 17 | QA false negatives |
| 16 | Approval | P0 | M4 | One "Approve & Print"; exact book locked | Deep-immutable `ApprovedBookRevision` + hash | 15, 10 | Lock semantics leaks |
| 17 | Print rendering | P0 | M4 | Deterministic, print-ready output | Bleed/safe/DPI/fonts/cover/spine pipeline; `PrintProvider` | 14, 16 | Printer PDF standard |
| 18 | Cart & checkout | P0 | M5 | Multi-book cart, pay in seconds | Medusa `CommerceModule`; line item → approved revision | 16 | Medusa edition; tax/VAT |
...
| 25 | Privacy & deletion | P1 | M6 (full) | Explicit windows + delete-now | Data inventory, retention cascade, consent in-flow | 04, 05 | Provider delete-API gaps |
...
| 28 | Failure recovery | P1 | M2 (core) | Failures recover; nothing silently lost | Idempotency, backoff, leases, one-page isolation | 10 | Queue choice (Temporal vs DB) |

## B. Platform decisions

| Primitive | Decision | Purpose | Boundary |
| --- | --- | --- | --- |
| **Medusa** | EVALUATE → likely ADOPT (spike: edition/hosting) | Commerce: cart, customer, product, pricing, payment, order, regions, currency, shipping | NEVER the Book model owner; line items reference `approvedBookRevisionId` |
| **OpenPolotno `@reyka/openpolotno`** | EVALUATE / WRAP (spike: spread, serialization) | Low-level editor engine under a custom simple UI | Never canonical model; never full Canva UX; pinned/possibly forked |
...
| **Temporal (et al.)** | REJECT BY DEFAULT; spike to confirm | Durable execution | DB-backed queue default for v1 |
...
- Print: deterministic artifact; exact approved revision preserved (F-016/F-017).
- Privacy: photo retention windows + delete-now enforced end-to-end (F-025).
