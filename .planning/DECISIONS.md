...
**Status:** OPEN for the remaining spikes; **SUPERSEDED for the durable job substrate**, which Spike A resolved to **ADOPT pg-boss** on 2026-09-22 (measured evidence in `RESEARCH_LOG.md`; `spike/durable-execution/`). Spike E's measurement methodology landed 2026-09-22 and Spike B's headless editor validation landed 2026-09-22 (see the bullets below) but their decisions remain OPEN. `OPEN` is recorded deliberately: insufficient evidence must never become a fake decision.
...
- **Editor implementation posture.** OPEN — Spike B tests the candidate editor against an actual children's-book spread (dimensions, multipage, serialization/restoration, undo/redo, bundle size, canonical-model adapter). Decide: depend directly / pin / wrap / maintain an internal fork / reject. The editor snapshot must never become canonical. **Update (2026-09-22):** Spike B phase 1 landed the headless validation (`spike/editor-primitive/`, 26/26 tests, acceptance path PASS for both `openpolotno@1.0.2` and `@reyka/openpolotno@1.5.0`, `RESEARCH_LOG.md` entry) — both stores run the canonical Book → adapter → snapshot → edit → canonical command → discard → rebuild round-trip entirely in Node, and both ship zero `.d.ts` files (type shim + pinned deep subpath required regardless of posture). This item stays OPEN: the posture (depend/pin/wrap/fork/reject) needs a browser render + phone-surface interaction pass before it can close; real font metrics and touch latency are still unmeasured.
