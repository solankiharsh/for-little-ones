...
1. **Durable projects and generation jobs — shipped in PR #25.** Draft/revision state now lives in an anonymous server-owned project with a hashed ownership token; story and concept attempts persist as generation jobs and the teaser restores from the server after refresh.
2. **Story discovery and correction — in build in PR #25.** Three bounded, moderated concepts, durable selection, authored fallbacks, provider timeout, measured token usage and age-specific writing policy are implemented. Paid page-level correction remains the next slice and must preserve all unchanged pages.
